...
/**
 * Offline indexer for systemui scope (com.android.systemui).
 *
 * Migrated as-is from DexKit queries in the following Hooks:
 * - NoChargeAnimation (ChargingAnimationController Handler field name)
 * - SystemUINetworkSpeeddoublelayerHook (NetworkSpeedView Handler inner class name)
 */
    ...
    private fun indexNoChargeAnimation(bridge: DexKitBridge): JsonObject {
        ...
        try {
            ...
            if (classData != null) {
                ...
                // 1) Prefer Handler type fields (including $ inner class variants)
                ...
                // 2) Fallback: first non-primitive field not starting with java./android.
