 ...
 *
 * Appends real-time charging info (power, voltage, current, temperature) to lock screen charging indication.
 *
 * Read strategy: prefers Java IO to directly read sysfs (SystemUI runs as system uid,
 * usually has permission), automatically falls back to su command on failure.
 *
 * Display format is configurable via frontend sub-toggles or advanced custom format (placeholder substitution, zero shell risk).
 */
...
class SystemUIRealWatts : AppHookModule() {
                ...
                runCatching {
                    ...
                    if (chargingData != null && chargingData.isCharging && chargingData.power > 0) {
                        ...
                        if (displayText.isEmpty()) {
                            logger.warn("Failed to detect charging power")
                            return@runCatching "$originalText\n --W"
                        }
                        lastUpdate = System.currentTimeMillis()
                        logger.debug("Successfully added charging display: $displayText")
                        originalText + "\n" + displayText
                    } else {
                        logger.warn("Failed to detect charging power")
                        "$originalText\n --W"
    ...
    /** Read charging data: Java IO preferred, fallback to su on failure. */
    private fun readChargingData(): ChargingData? {
        readChargingDataViaFileIO()?.let { return it }

        logger.warn("Java IO failed to read sysfs, attempting fallback to su mode")
        if (!isSuAvailable()) {
            logger.warn("su not available, cannot fallback")
            return null
        }
    ...
    private fun readChargingDataViaFileIO(): ChargingData? {
        return try {
            ...
            if (currentStr.isNullOrEmpty() || voltageStr.isNullOrEmpty()) {
                logger.warn("Java IO read sysfs has no valid data - current: $currentStr, voltage: $voltageStr")
                return null
            }
            buildChargingData(status, currentStr, voltageStr, tempStr, "Java IO")
        } catch (e: Exception) {
            logger.error("Java IO exception while reading charging data", e)
            null
        }
    }
    ...
    private fun readChargingDataViaSu(): ChargingData? {
        return try {
            ...
            logger.error("su exception while reading charging data", e)
            null
        }
    }
    ...
    private fun buildChargingData(
        ...
        if (!tempStr.isNullOrEmpty()) {
            try {
                ...
                logger.warn("Temperature value parse failed: $tempStr")
            }
        }
    ...
    private fun readSysfs(path: String): String? {
        return try {
            ...
            logger.warn("Java IO failed to read $path: ${e.message}")
            null
        }
    }
    ...
    private fun isSuAvailable(): Boolean {
        ...
        return try {
            ...
            logger.warn("su availability check exception: ${e.message}")
            suAvailable = false
            false
        }
    }
    ...
    private fun executeRootCommand(command: String): String? {
        return try {
            ...
            if (exitCode != 0) {
                logger.warn("su command execution failed, exit code: $exitCode, command: $command")
                return null
            }
            logger.debug("su command executed successfully: $command -> $output")
            output
        } catch (e: Exception) {
    ...
    private data class ChargingData(
        ...
        val voltage: Float,     // V
        val power: Double,      // W
        val temperature: Double // °C, -273 indicates invalid
    )
}
