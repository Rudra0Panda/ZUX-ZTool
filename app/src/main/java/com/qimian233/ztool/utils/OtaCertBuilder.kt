...
/**
 * Pure Java (no BouncyCastle) construction of self-signed X.509 v3 certificate,
 * used for local OTA re-signing otacerts.zip trust chain.
 *
 * Only supports RSA keys + SHA256withRSA (consistent with update_engine payload_verifier
 * otacerts verification path). Certificate is generated and used on-device only, CN marked as ZTool.
 *
 * DER structure adheres to RFC 5280:
...
object OtaCertBuilder {
    ...
    /**
     * Generate self-signed certificate using existing RSA key pair, returning X.509 DER encoding.
     * SubjectPublicKeyInfo for the public key reuses [publicKey].encoded directly without re-encoding.
     */
    ...
    /**
     * Verify that generated certificate can be parsed by Android's CertificateFactory,
     * returning the parsed X509Certificate (throws CertificateException on failure).
     */
    @Throws(CertificateException::class)
    fun toX509Certificate(der: ByteArray): X509Certificate {
        val cert = CertificateFactory.getInstance("X.509")
            .generateCertificate(der.inputStream()) as X509Certificate
        // Actively trigger validation to discover DER construction errors early
        cert.checkValidity()
        return cert
    }

    /** Extract public key from X.509 DER (used for reverse verification that certificate matches key pair). */
    ...
    private fun derUtcTime(calendar: Calendar): ByteArray {
        // UTCTime only supports up to 2049; notAfter beyond 2049 requires GeneralizedTime,
        // notAfter = now + 20 years, clamped to 2049-12-31 if out of range.
    ...
    private fun randomSerial(): ByteArray {
        ...
        // Ensure positive integer: clear highest bit
